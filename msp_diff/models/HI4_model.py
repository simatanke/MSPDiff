...
class HI4(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.net_le_2 = build_network(opt['network_le'])
        self.net_le_2 = self.model_to_device(self.net_le_2)
        self.print_network(self.net_le_2)
    ...
    def setup_optimizers(self):
        ...
        for k, v in self.net_le_2.named_parameters():
            if v.requires_grad:
                optim_params.append(v)
            else:
                logger = get_root_logger()
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.prior = self.net_le(self.lq, self.gt)

        self.prior1 = self.net_le_2(self.lq,self.lq)
        ...
        if self.cri_pix_diff:
            l_pix_diff = self.cri_pix_diff(self.prior, self.prior1)
            l_total += l_pix_diff
            loss_dict['l_pix_diff'] = l_pix_diff
        # perceptual loss
        ...
        if self.opt['train']['use_grad_clip']:
            torch.nn.utils.clip_grad_norm_(list(self.net_g.parameters()) + list(self.net_le_2.parameters()), 0.01)
    ...
    def save(self, epoch, current_iter):
        if hasattr(self, 'net_g_ema'):
            ...
            self.save_network(self.net_le_2, 'net_le_2', current_iter)
