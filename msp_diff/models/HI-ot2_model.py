...
class HI_ot2(BaseModel):
    """ model for model for ablation"""

    def __init__(self, opt):
        super(HI_ot2, self).__init__(opt)

        # define network
        self.net_le_dm2= build_network(opt['network_le_dm'])
        self.net_le_dm2= self.model_to_device(self.net_le_dm2)
        self.print_network(self.net_le_dm2)
    ...
    def setup_optimizers(self):
        ...
        for k, v in self.net_le_dm2.named_parameters():
            if v.requires_grad:
                optim_params.append(v)
            else:
                logger = get_root_logger()
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.prior = self.net_le_dm(self.gt)

        self.prior1 = self.net_le_dm2(self.lq)

        #self.output = self.net_g(self.lq, self.prior1)
        ...
        if self.cri_pix_diff:
            l_pix_diff = self.contentLoss.get_otloss(self.prior1, self.prior)
            l_total += l_pix_diff
        ...
        # l_total += self.latent_otLoss
        loss_dict['l_pix'] = l_total
        l_total.backward()
        if self.opt['train']['use_grad_clip']:
            torch.nn.utils.clip_grad_norm_(list(self.net_g.parameters()) + list(self.net_le_dm2.parameters()), 0.01)
    ...
    def test(self):
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            self.net_le_dm.eval()
            self.net_le_dm2.eval()
            self.net_g.eval()
            with torch.no_grad():
                prior = self.net_le_dm2(img)
    ...
    def save(self, epoch, current_iter):
        if hasattr(self, 'net_g_ema'):
            ...
            self.save_network(self.net_le_dm2, 'net_le_dm2', current_iter)
