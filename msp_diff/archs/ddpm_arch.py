...
class GaussianDiffusion(nn.Module):
    ...
    def set_new_noise_schedule(self, schedule_opt, device):
        ...
        assert schedule_opt['n_timestep'] % len(self.schedule_list) == 0
        segment_length = schedule_opt['n_timestep'] // len(self.schedule_list)
        downsampling_schedule = []
        for downsampling_scale in self.schedule_list:
    ...
    def ddim_pyramid_sample(
        ...
        assert len(
            schedule_list) == ddim_timesteps, f'len(schedule_list):{len(schedule_list)} != ddim_timesteps{ddim_timesteps}'
        ...
        init_h = h // schedule_list[-1]
        init_w = w // schedule_list[-1]
        ...
        for i in tqdm(reversed(range(0, ddim_timesteps)), desc='sampling loop time step', total=ddim_timesteps):
            ...
            if i != 0 and schedule_list[i] != schedule_list[i - 1]:
                upsample_scale = schedule_list[i] // schedule_list[i - 1]
